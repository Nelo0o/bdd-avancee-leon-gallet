...
export const scriptManager = {
  ...
  deleteEmployee: async (id) => {
    const conn = getConnection();
    try {
      // D'abord vérifier si le Didier existe
      const [employee] = await conn.query('SELECT id FROM employees WHERE id = ?', [id]);
  ...
  getServiceById: async (id) => {
    const conn = getConnection();
    const [results] = await conn.query('SELECT * FROM services WHERE id = ?', [id]);
    return results[0];
  },
